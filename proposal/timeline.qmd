...
Development will take place publicly in the GitHub repository: [atsyplenkov/wbw](https://github.com/atsyplenkov/wbw), owned by Anatolii Tsyplenkov. The project package will be licensed compatibly with R and `terra`/`sf` (MIT or GPL >= 2). The project leader will provide the R Consortium with monthly updates based on progress toward the deliverables below.
<!--JN: I would suggest specifying one of the licenses-->
...
<!-- JN: I know that the current plan fits nicely, but the questions is: how long it takes to get the decision and the funding? Would it be possible before May 1st? -->
...
<!-- JN: I would suggest removing this sentence: "There is also a possibility of fully transitioning to the `testthat` infrastructure, if needed." -->
...
<!-- JN: (possibly silly) question: given that most of the functions follow a similar structure -- cannot they be implemented by writing some "metafunction" that creates them?   -->
